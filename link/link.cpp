...
int main() {
    try {
        ...
        std::function<void(const boost::system::error_code&,std::size_t)> udpHandler =
            [&udpHandler, &rxSocket, &rxEndpoint, &rxbuf, &serial](const boost::system::error_code& error, std::size_t bytes_transferred) {
                if (error.failed()) {
                    ...
                    if (bytes_transferred > 4) {
                        std::cout << "Received TC[" << int{rxbuf[1]} << "," << int{rxbuf[2]} << "] (" << bytes_transferred << " bytes)" << std::endl;
                    }

                    // COBS encoding
                    auto result = cobs_encode(cobs_buffer.data(), cobs_buffer.size() - 1, rxbuf.data(), bytes_transferred);

                    if (result.status != COBS_ENCODE_OK) {
                        std::cerr << "COBS encode status returned " << static_cast<int>(result.status) << std::endl;
                    } else {
        ...
        std::function<void(const boost::system::error_code&,std::size_t)> serialHandler =
            [&serial, &serialHandler, &buf, &txSocket, &endpoint](const boost::system::error_code& error, std::size_t bytes_transferred) -> void {
